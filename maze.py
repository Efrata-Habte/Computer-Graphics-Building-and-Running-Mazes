...
BLACK = (35, 35, 35)
RED = (255, 0, 0)
YELLOW = (255, 255, 0)  # Changed to yellow
GREEN = (0, 255, 0)  # Restored to green
ROWS = 25
COLS = 25
    ...
    def refresh_view(self, screen, current=None):
        ...
        for r, c in self.dead_end_markers:
            pygame.draw.rect(
                screen,
                YELLOW,
                (c * CELL_SIZE + 4, r * CELL_SIZE + 4, CELL_SIZE - 8, CELL_SIZE - 8),
            )
        ...
        for r, c in self.final_route:
            pygame.draw.rect(
                screen,
                GREEN,
                (c * CELL_SIZE + 6, r * CELL_SIZE + 6, CELL_SIZE - 12, CELL_SIZE - 12),
            )
        ...
        for r in range(self.rows):
            if self.east_wall[r][0]:
                pygame.draw.line(
                    screen, WHITE, (0, r * CELL_SIZE), (0, (r + 1) * CELL_SIZE), 2
                )
    ...
    def generate_labyrinth(self, screen):
        ...
        while stack:
            ...
            if neighbors:
                ...
                # "Eat" the wall
                if direction == "N":
                    self.north_wall[curr_r][curr_c] = 0
                elif direction == "S":
                    self.north_wall[nr][nc] = 0
                elif direction == "E":
                    self.east_wall[curr_r][curr_c + 1] = 0
                elif direction == "W":
                    self.east_wall[curr_r][curr_c] = 0
    ...
    def solve_labyrinth(self, screen):
        ...
        while solve_stack:
            ...
            if r > 0 and not self.north_wall[r][c]:
                accessible.append((r - 1, c))
            if r < self.rows - 1 and not self.north_wall[r + 1][c]:
                accessible.append((r + 1, c))
            if c > 0 and not self.east_wall[r][c]:
                accessible.append((r, c - 1))
            if c < self.cols - 1 and not self.east_wall[r][c + 1]:
                accessible.append((r, c + 1))
